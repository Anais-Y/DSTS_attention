...
class gcn_operation(nn.Module):
    def __init__(self, adj, in_dim, out_dim, strides, activation='GLU'):
        """
        图卷积模块
        :param in_dim: 输入维度
        :param out_dim: 输出维度
    ...
    def forward(self, x):
        ...
        # print('before', x.device, adj.device)
        x = torch.einsum('nm, mbc->nbc', adj, x)
        # print('after', x.device, adj.device)
        if self.activation == 'GLU':
...
class STSGCM(nn.Module):
    def __init__(self, adj, in_dim, out_dims, num_of_vertices, strides, activation='GLU'):
        """
        :param adj: 邻接矩阵
        :param in_dim: 输入维度
...
class STSGCL(nn.Module):
    def __init__(self,
        ...
        self.adj = nn.Parameter(torch.FloatTensor(num_of_vertices, num_of_vertices))  # 在每个module里面初始化一次adj
        nn.init.xavier_uniform_(self.adj)
        self.weight_cross_time = nn.Parameter(torch.FloatTensor(strides - 1, num_of_vertices, num_of_vertices))
        nn.init.xavier_uniform_(self.weight_cross_time)
        self.constructed_adj = self.construct_adj()
