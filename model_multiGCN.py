...
class gcn_operation(nn.Module):
    def __init__(self, fc, activation='GLU'):
        """
        图卷积模块
...
class STSGCM(nn.Module):
    def __init__(self, fc, num_of_vertices, activation='GLU'):
        """
        :param adj: 邻接矩阵
        :param in_dim: 输入维度
        ...
        self.activation = activation
        # self.constructed_adj = constructed_adj
        self.FC = fc
        self.gcn_operation = gcn_operation(fc, activation=activation)
...
class STSGCL(nn.Module):
    def __init__(self,
        ...
        self.num_of_vertices = num_of_vertices
        # self.constructed_adj = BlockMatrix(num_of_vertices, strides)
        self.constructed_adj = nn.ModuleList([BlockMatrix(num_of_vertices, strides) for _ in range(num_gcn)])
        self.num_gcn = num_gcn

        self.shared_fc = nn.ModuleList([nn.Linear(in_dim, out_dims[k], bias=True) for k in range(num_gcn)])
        self.FC_kernel = nn.Linear(self.num_gcn, 1)
        self.activation = activation
        self.temporal_emb = temporal_emb
        self.spatial_emb = spatial_emb

        self.STSGCMS = nn.ModuleList([STSGCM(fc=self.shared_fc[k], num_of_vertices=self.num_of_vertices,
                                             activation=self.activation) for k in range(num_gcn)])
    ...
    def forward(self, x):
        ...
        for k in range(self.num_gcn):
            adj = self.constructed_adj[k]()
            need_concat = []
            batch_size = x.shape[0]
            for i in range(self.history - self.strides + 1):
                # constructed_adj = self.constructed_adj
        ...
        all_kernel = torch.stack(sum_out, dim=4)  # (B, T-2, N, Cout, num_gcn)
        agg = self.FC_kernel(all_kernel)  # (B, T-2, N, Cout, 1)
        layer_output = agg.squeeze(-1)  # (B, T-2, N, Cout)
