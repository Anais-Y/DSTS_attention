        ...
        for chan in range(chans):
            for band in range(bands):
                ...
                for i in range(9):
                    ...
                    features[seg, i, chan, band] = normed
    return features
...
def generate_train_val_test_shuffle(seperated, labels, save_folder):
    ...
    for cat in ['train', 'val', 'test']:
        ...
        np.savez_compressed(
            # 保存多个数组，按照你定义的key字典保存，compressed表示它是一个压缩文件
            os.path.join(save_folder, f"{cat}_LDS.npz"),
            x=_x,
            y=_y)
...
if __name__ == '__main__':
    ...
    file_path = f'./data/data_preprocessed_matlab/s06.mat'
    raw_data, new_label = load_data_label_regression(file_path)
    filtered = filter_data(raw_data)
    baseline = compute_baselineDE(filtered)
    window_num = 9  # 需被T整除
    features = compute_features_96(filtered, baseline)
    features_LDS = filterLDS(features)
    # seperated, labels = seperate(features, 9, new_label)
    labels = np.repeat(new_label, 30, axis=0)
    print(features_LDS.shape, labels.shape)
    save_folder = f'./data/processed/s06_removed_LDS'
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    generate_train_val_test_shuffle(features_LDS, labels, save_folder)
