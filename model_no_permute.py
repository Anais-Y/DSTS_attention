...
class gcn_operation(nn.Module):
    def __init__(self, in_dim, out_dim, num_vertices, strides, activation='GLU'):
        ...
        self.bn = nn.BatchNorm1d(3*self.num_vertices)
    ...
    def forward(self, x):
        ...
        x = torch.einsum('nm, bmc->bnc', adj.to(x.device), x)  # B, 3N, Cin
        if self.activation == 'GLU':
...
class STSGCM(nn.Module):
    ...
    def forward(self, x):
        """
        :param x: (3N, B, Cin) (B, 3N, Cin)
        :return: (N, B, Cout) (B
        """
        need_concat = []

        for i in range(len(self.out_dims)):
            x = self.gcn_operations[i](x)  # B, 3N, Cout
            need_concat.append(x)

        # shape of each element is (1, B, N, Cout)
        need_concat = [
            torch.unsqueeze(
        ...
        out = torch.max(torch.cat(need_concat, dim=0), dim=0).values  # (B, N, Cout)
...
class STSGCL(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.history - self.strides + 1):
            ...
            t = self.STSGCMS[i](t)  # t: (B, 3*N, Cin)

            #  old (3*N, B, Cin) -> (N, B, Cout)

            t = torch.unsqueeze(t, dim=1)  # (B, N, Cout) ->(B, 1, N, Cout)
...
class output_layer(nn.Module):
    def __init__(self, num_of_vertices, history, in_dim,
        ...
        # self.FC1 = nn.Linear(self.in_dim * self.history * self.num_of_vertices, self.hidden_dim, bias=True)
        self.self_attention = SelfAttention(in_dim * history, hidden_dim)

        self.FC2 = nn.Linear(self.hidden_dim * self.num_of_vertices, 2, bias=True)
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch_size, self.num_of_vertices * self.hidden_dim)  # Reshape to (B, N*hidden_dim)

        out2 = self.FC2(x)  # (B, N*hidden_dim) -> (B, 2)
        return out2
