...
# in_dim-->out_dims[0]-->out_dims[1]-->out_dims[...]-->out_dims[n-1]
    ...
    def forward(self, x, constructed_adj):
        ...
        for i in range(len(self.out_dims)):
            x = self.gcn_operations[i](x, constructed_adj)
...
class STSGCL(nn.Module):
    ...
    def forward(self, x, constructed_adj):
        ...
        for i in range(self.history - self.strides + 1):
            ...
            t = self.STSGCMS[i](t.permute(1, 0, 2), constructed_adj)  # (3*N, B, Cin) -> (N, B, Cout)
...
class STSGCN(nn.Module):
    def __init__(self, history, num_of_vertices, in_dim, hidden_dims,
        ...
        self.constructed_adj = nn.Parameter(torch.FloatTensor(3*num_of_vertices, 3*num_of_vertices))
        # self.weight_cross_time = nn.Parameter(torch.FloatTensor(strides - 1, num_of_vertices, num_of_vertices))
        nn.init.xavier_uniform_(self.constructed_adj)
    ...
    def forward(self, x):
        ...
        for model in self.STSGCLS:
            x = model(x, self.constructed_adj)
