...
class STSGCL(nn.Module):
    def __init__(self,
        ...
        self.shared_fc = nn.Linear(in_dim, out_dims[0], bias=True)

        self.activation = activation
        self.temporal_emb = temporal_emb
        self.spatial_emb = spatial_emb

        self.STSGCM = STSGCM(fc=self.shared_fc, num_of_vertices=self.num_of_vertices, activation=self.activation)
    ...
    def forward(self, x):
        ...
        for i in range(self.history - self.strides + 1):
            ...
            t = self.STSGCM(t.permute(1, 0, 2), constructed_adj)  # (3*N, B, Cin) -> (N, B, Cout)
