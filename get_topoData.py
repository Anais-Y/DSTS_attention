...
def main():
    ...
    numpy_y = y.numpy()
    eu_dist = np.zeros((numpy_y.shape[0], 3))
    eu_dist[:, 0] = numpy_y[:, 0]
    eu_dist[:, 1] = numpy_y[:, 1]
    eu_dist[:, 2] = np.array([euclidean(point, [5, 5]) for point in numpy_y])
    print(eu_dist)
    np.save(f'./Topo_data/{DATASET}-label.npy', eu_dist)
    # -----------LVLA---------#
    type = 'LVLA'

    filtered_indices = np.where((eu_dist[:, 0] < low) & (eu_dist[:, 1] < low))
    max_value_index = np.argmax(eu_dist[filtered_indices][:, 2])
    index = filtered_indices[0][max_value_index]
    print(eu_dist[index])

    x_LVLA = torch.unsqueeze(x[index, ...], dim=0)
    with torch.no_grad():
        output = model(x_LVLA).numpy()
        output = np.squeeze(output)
        np.save(f'./Topo_data/{DATASET}-{type}.npy', output)

    # -----------HVHA---------#
    type = 'HVHA'

    filtered_indices = np.where((eu_dist[:, 0] > high) & (eu_dist[:, 1] > high))
    max_value_index = np.argmax(eu_dist[filtered_indices][:, 2])
    index = filtered_indices[0][max_value_index]
    print(eu_dist[index])

    x_HVHA = torch.unsqueeze(x[index, ...], dim=0)
    with torch.no_grad():
        output = model(x_HVHA).numpy()
        output = np.squeeze(output)
        np.save(f'./Topo_data/{DATASET}-{type}.npy', output)
    ...
    filtered_indices = np.where((eu_dist[:, 0] < low) & (eu_dist[:, 1] > high))
    max_value_index = np.argmax(eu_dist[filtered_indices][:, 2])
    index = filtered_indices[0][max_value_index]
    print(eu_dist[index])

    x_LVHA = torch.unsqueeze(x[index, ...], dim=0)
    with torch.no_grad():
        output = model(x_LVHA).numpy()
        output = np.squeeze(output)
        print(output.shape)
        np.save(f'./Topo_data/{DATASET}-{type}.npy', output)
    ...
    filtered_indices = np.where((eu_dist[:, 0] > high) & (eu_dist[:, 1] < low))
    max_value_index = np.argmax(eu_dist[filtered_indices][:, 2])
    index = filtered_indices[0][max_value_index]
    print(eu_dist[index])

    x_HVLA = torch.unsqueeze(x[index, ...], dim=0)
    with torch.no_grad():
        output = model(x_HVLA).numpy()  # 这一行会print出model(x_HVLA)原来的shape
        output = np.squeeze(output)
        print(output.shape)
        np.save(f'./Topo_data/{DATASET}-{type}.npy', output)
